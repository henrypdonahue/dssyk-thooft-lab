...
## Validation (all asserted in `test_thooft.py`, 20 tests)
...
- The 12-digit guarantee is an $\alpha=0$ statement. Off the duality point the basis
  hardwires the wrong endpoint exponent, so convergence is algebraic; the measured
  limitation is the 1e-5–2e-4 residual in the LM sum-rule tests. (A Jacobi
  $[x(1-x)]^\beta$ basis would restore spectral accuracy there.)

## Files & usage

| file | purpose |
|------|---------|
| `thooft_spectrum.py` | main double-precision solver (takes `alpha`) |
...
| `test_thooft.py` / `validate.py` | asserting suite / human-readable gate (exit code) |
...
pytest -q                      # 20 asserting tests (~6 s)
```
