...
\paragraph{Double scaling: Eq.~(3.28) for the canonical adjoint bilinear, exactly.}
...
bound of \cite{MSS2607} holds \emph{for this observable}, verified by exact
computation in the regime the bound is about (we have evaluated Eq.~\eqref{eq:var} to $N=1600$, $p=40$),
...
four-point function of \cite{BINT}. A companion note proves that at strict
double scaling the bilinear channels are conserved (the chord
representation of each $M_n$ analog is a polynomial in the chord
Hamiltonian), so that extraction is necessarily a $1/N$ computation,
performed at fixed $p$ --- where the present note quantifies the accuracy
of the emergent gauge symmetry.
